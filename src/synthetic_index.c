...
int main() {
    ...
    while (fgetws(line, MAX_LINE_LENGTH, input_file)) {
        ...
        // Check for quoted sentences
        wchar_t *start_quote = wcschr(line, L'"');
        wchar_t *end_quote = wcsrchr(line, L'"');
        if (start_quote && end_quote && start_quote != end_quote) {
            *end_quote = L'\0';
            wcscpy_s(sentence, MAX_SENTENCE_LENGTH, start_quote + 1);

            wchar_t *data = end_quote + 1;
            while (*data == L',' || *data == L' ') {
                data++;
            }

            if (swscanf_s(data, L"%d,%d,%d,%d", &words, &morphemes, &inflectional_morphemes, &function_words) == 4) {
            ...
            // Process non-quoted sentences
            if (swscanf_s(line, L"%[^,],%d,%d,%d,%d", sentence, (unsigned)_countof(sentence), &words, &morphemes, &inflectional_morphemes, &function_words) == 5) {
                wprintf(L"Sentence: %ls\n", sentence);
                wprintf(L"Words: %d, Morphemes: %d, Inflectional: %d, Function: %d\n", words, morphemes, inflectional_morphemes, function_words);
                process_sentence(output_file, sentence, words, morphemes, inflectional_morphemes, function_words);
            } else {
